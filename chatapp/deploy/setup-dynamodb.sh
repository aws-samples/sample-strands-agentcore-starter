#!/bin/bash
# Set up DynamoDB table for usage analytics
# Usage: ./setup-dynamodb.sh [--yes]
#
# Options:
#   --yes    Auto-confirm all prompts (non-interactive mode)
#
# This script creates:
# - DynamoDB table for storing usage records
...
TABLE_NAME="${USAGE_TABLE_NAME:-agentcore-usage-records}"

echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║         DynamoDB Setup for Usage Analytics                 ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if aws iam get-role --role-name "$TASK_ROLE_NAME" 2>/dev/null; then
    ...
    aws iam put-role-policy \
        ...
        --policy-name "DynamoDBUsageAccess" \
        --policy-document "$DYNAMODB_POLICY"
    
    echo -e "${GREEN}Updated task role with DynamoDB permissions${NC}"
else
...
echo ""
echo "USAGE_TABLE_NAME=${TABLE_NAME}"
echo ""
echo -e "${YELLOW}Next Steps:${NC}"
echo "1. Update your chatapp/.env file with USAGE_TABLE_NAME"
...
if [ -f "$ENV_FILE" ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        if grep -q "^USAGE_TABLE_NAME=" "$ENV_FILE"; then
            sed -i.bak "s|^USAGE_TABLE_NAME=.*|USAGE_TABLE_NAME=${TABLE_NAME}|" "$ENV_FILE"
        else
            echo "USAGE_TABLE_NAME=${TABLE_NAME}" >> "$ENV_FILE"
        fi
        rm -f "$ENV_FILE.bak"
