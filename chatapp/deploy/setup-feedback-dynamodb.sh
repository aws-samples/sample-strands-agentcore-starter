#!/bin/bash
# Set up DynamoDB table for user feedback
# Usage: ./setup-feedback-dynamodb.sh [--yes]
#
# Options:
#   --yes    Auto-confirm all prompts (non-interactive mode)
#
# This script creates:
# - DynamoDB table for storing feedback records
...
TABLE_NAME="${FEEDBACK_TABLE_NAME:-agentcore-feedback}"

echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║         DynamoDB Setup for User Feedback                   ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if aws iam get-role --role-name "$TASK_ROLE_NAME" 2>/dev/null; then
    # Create inline policy for DynamoDB feedback access
    DYNAMODB_POLICY="{
      \"Version\": \"2012-10-17\",
      \"Statement\": [
        {
          \"Sid\": \"DynamoDBFeedbackAccess\",
          \"Effect\": \"Allow\",
          \"Action\": [
            \"dynamodb:PutItem\",
            \"dynamodb:GetItem\",
    ...
    aws iam put-role-policy \
        ...
        --policy-name "DynamoDBFeedbackAccess" \
        --policy-document "$DYNAMODB_POLICY"
    
    echo -e "${GREEN}Updated task role with DynamoDB feedback permissions${NC}"
else
...
echo ""
echo -e "${GREEN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║         DynamoDB Feedback Setup Complete!                  ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo "FEEDBACK_TABLE_NAME=${TABLE_NAME}"
echo ""
echo -e "${YELLOW}Next Steps:${NC}"
echo "1. Update your chatapp/.env file with FEEDBACK_TABLE_NAME"
...
if [ -f "$ENV_FILE" ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        if grep -q "^FEEDBACK_TABLE_NAME=" "$ENV_FILE"; then
            sed -i.bak "s|^FEEDBACK_TABLE_NAME=.*|FEEDBACK_TABLE_NAME=${TABLE_NAME}|" "$ENV_FILE"
        else
            echo "FEEDBACK_TABLE_NAME=${TABLE_NAME}" >> "$ENV_FILE"
        fi
        rm -f "$ENV_FILE.bak"
